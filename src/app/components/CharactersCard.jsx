...
export default function CharactersCard({characters}) {
  ...
  return (
    <div className='flex justify-center py-5'>
        <div className="flex gap-3 w-1/3 justify-center flex-wrap">
          {characters.map(char => (
            <button className="w-32 h-32" key={char.name} onClick={()=>handleCharacter(char)} value={char.name}>
                <img src={char.avatar} className='w-32 opacity-30 hover:opacity-100 h-32 hover:scale-110 duration-300 cursor-pointer focus:opacity-100 square' />
            </button>
          ))}
        </div>
    
    <div className="w-1/3 flex flex-col gap-5 ">
          <h2 className="text-2xl">{characterInfo.name}</h2>
